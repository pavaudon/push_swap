...
void	ft_data_stack(char **argv, t_both *both)
{
	...
	{
		cpt++;
		both->the_min = (both->the_min > ft_atoi(argv[i])) ?
		ft_atoi(argv[i]) : both->the_min;
		both->the_max = (both->the_max < ft_atoi(argv[i])) ?
		ft_atoi(argv[i]) : both->the_max;
	}
	both->the_size = cpt;
	SIZE_A = cpt;
	SIZE_B = 0;
...
int		ft_checker(char **argv)
{
	...
	{
		printf("command : '%s'\n", command);
		if (!command || (ft_strlen(command) < 2 || ft_strlen(command) > 5) ||
		!ft_is_command(command, both, ft_strlen(command)))
		{
